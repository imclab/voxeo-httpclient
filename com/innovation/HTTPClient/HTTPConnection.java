...
package com.innovation.HTTPClient;
...
public class HTTPConnection implements GlobalConstants, HTTPClientModuleConstants
{
	...
	try
	{
	    String hosts = System.getProperty("com.innovation.HTTPClient.nonProxyHosts");
	    if (hosts == null)
	...
	try
	{
	    String host = System.getProperty("com.innovation.HTTPClient.socksHost");
	    if (host != null  &&  host.length() > 0)
	    {
		int port    = Integer.getInteger("com.innovation.HTTPClient.socksPort", -1).intValue();
		int version = Integer.getInteger("com.innovation.HTTPClient.socksVersion", -1).intValue();
	...
	String modules = "com.innovation.HTTPClient.RetryModule|" +
			 "com.innovation.HTTPClient.CookieModule|" +
			 "com.innovation.HTTPClient.RedirectionModule|" +
			 "com.innovation.HTTPClient.AuthorizationModule|" +
			 "com.innovation.HTTPClient.DefaultModule|" +
			 "com.innovation.HTTPClient.TransferEncodingModule|" +
			 "com.innovation.HTTPClient.ContentMD5Module|" +
			 "com.innovation.HTTPClient.ContentEncodingModule";

	boolean in_applet = false;
	try
	    { modules = System.getProperty("com.innovation.HTTPClient.Modules", modules); }
	...
	try
	{
	    neverPipeline = Boolean.getBoolean("com.innovation.HTTPClient.disable_pipelining");
	    if (neverPipeline)
	...
	try
	{
	    noKeepAlives = Boolean.getBoolean("com.innovation.HTTPClient.disableKeepAlives");
	    if (noKeepAlives)
	...
	try
	{
	    force_1_0 = Boolean.getBoolean("com.innovation.HTTPClient.forceHTTP_1.0");
	    if (force_1_0)
	...
	try
	{
	    no_chunked = Boolean.getBoolean("com.innovation.HTTPClient.dontChunkRequests");
	    if (no_chunked)
	...
	try
	{
	    deferStreamed = Boolean.getBoolean("com.innovation.HTTPClient.deferStreamed");
	    if (deferStreamed)
     ...
     * @exception ModuleException if an exception is encountered in any module.
     */
    public HTTPResponse Purge(String file)
		throws IOException, ModuleException
    {
	return setupRequest("PURGE", file, null, null, null);
    }
	...
	String[] modules = { "com.innovation.HTTPClient.CookieModule",
			     "com.innovation.HTTPClient.RedirectionModule",
			     "com.innovation.HTTPClient.AuthorizationModule",
			     "com.innovation.HTTPClient.DefaultModule",
			     "com.innovation.HTTPClient.TransferEncodingModule",
     ...
     * <P>Example:
     * <PRE>
     * HTTPConnection.addDefaultModule(Class.forName("com.innovation.HTTPClient.CookieModule"), 1);
     * </PRE>
     ...
     *
     * "com.innovation.HTTPClient.RetryModule | HTTPClient.CookieModule |
	...
	if (con_timeout == 0)		// normal connection establishment
	{
		...
		for (int idx=0; idx<addr_list.length; idx++)
		{
			...
			break;		// success
                        ...
                        //System.out.println("getSocket - timeout=" + Timeout);
                        sock = new Socket();  
                        java.net.SocketAddress endpoint = new java.net.InetSocketAddress(addr_list[idx], actual_port);
                        sock.connect(endpoint, Timeout); //this timeout works!
                        break;		// success
		    }
		...
		else
		{
			...
			try
			{
			    ...
			    break;		// success
                            ...
                            //System.out.println("Established Connection - timeout=" + Timeout);
